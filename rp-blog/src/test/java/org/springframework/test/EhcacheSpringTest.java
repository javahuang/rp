...
public class EhcacheSpringTest {
	...
	@Test
	public void test(){
		Cache sysCache = cacheManager.getCache("userCache");
		User u=new User();
		u.setAge(100);
		u.setName("小明");
		sysCache.put("u",u);
		//[ key = bane, value=小明, version=1, hitCount=1, CreationTime = 1430129540214, LastAccessTime = 1430129540275 ]
		//CacheUtils.getCacheManager()是可以直接获得spring创建的cacheManager实例的
		System.out.println(CacheUtils.getCacheValueAttribute("userCache", "u", "name",null));;
	}
	//@Test
	public void test1() {
