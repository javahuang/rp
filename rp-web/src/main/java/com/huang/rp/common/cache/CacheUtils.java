...
public class CacheUtils {
	...
	public void init(){
		...
		List<SysParameter> sysParameterList=sysParameterMapper.selectByExample(parameterExample);
		Cache parameterCache=getCacheManager().getCache(Constants.CACHE_SYS_PARAMETER);
		Map<String,List<SysParameter>> paraMap=Maps.newHashMap();
		for(SysParameter para:sysParameterList){//缓存的key为paraType value为SysParameter对象
		...
		for(Map.Entry<String, List<SysParameter>> paraList:paraMap.entrySet()){
			parameterCache.put(new Element(paraList.getKey(), paraList.getValue()));
		}
	}
